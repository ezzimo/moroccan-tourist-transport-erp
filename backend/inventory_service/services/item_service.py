...
class ItemService:
    ...
    async def get_items(
        self, pagination: PaginationParams, search: Optional[ItemSearch] = None
        ...
        if search:
            ...
            if search.warehouse_location:
                conditions.append(
                    Item.warehouse_location.ilike(
                        f"%{search.warehouse_location}%"
                    )
                )

            if search.supplier_id:
                conditions.append(
                    Item.primary_supplier_id == search.supplier_id
                )
    ...
    async def update_item(
        self, item_id: uuid.UUID, item_data: ItemUpdate
    ) -> ItemResponse:
    ...
    async def get_items_summary(self) -> ItemSummary:
        ...
        # Active items
        active_stmt = select(func.count(Item.id)).where(
        ...
        out_of_stock_stmt = select(func.count(Item.id)).where(
            and_(Item.status == ItemStatus.ACTIVE, Item.current_quantity <= 0)
        )
        ...
        # Total stock value
        value_stmt = select(
            func.sum(Item.current_quantity * Item.unit_cost)
        ).where(
            Item.status == ItemStatus.ACTIVE
        )
        ...
        # By warehouse
        warehouse_stmt = (
            select(Item.warehouse_location, func.count(Item.id))
            .where(Item.status == ItemStatus.ACTIVE)
        ...
        critical_low_stock_stmt = select(func.count(Item.id)).where(
            and_(
                Item.status == ItemStatus.ACTIVE,
                Item.is_critical is True,
                Item.current_quantity <= Item.reorder_level,
            )
        )
        critical_items_low_stock = self.session.exec(
            critical_low_stock_stmt
        ).one()
    ...
    async def get_low_stock_items(self) -> List[ItemResponse]:
        ...
        statement = (
            ...
            .order_by(Item.current_quantity / Item.reorder_level)
        )  # Most critical first
    ...
    async def get_reorder_suggestions(self) -> List[ItemReorderSuggestion]:
        ...
        for item in items:
            ...
            # (reorder to max level or 2x reorder level)
            if item.max_stock_level:
                suggested_quantity = (
                    item.max_stock_level - item.current_quantity
                )
            else:
                suggested_quantity = (
            ...
            suggestions.append(
                ItemReorderSuggestion(
                    ...
                    estimated_cost=(
                        suggested_quantity * item.unit_cost
                    ),
                    priority=priority,
                )
            )
    ...
    async def get_item_movements(
        self, item_id: uuid.UUID, limit: int = 50
    ) -> List[dict]:
