...
class AnalyticsService:
    ...
    async def get_revenue_analytics(
        ...
        for invoice in invoices:
            ...
            #     amount = await convert_currency(amount, invoice.currency, currency)
            ...
            monthly_revenue[month_key]["total"] += float(amount)
            monthly_revenue[month_key]["invoice_count"] += 1
            
            if invoice.status == InvoiceStatus.PAID:
                monthly_revenue[month_key]["paid"] += float(amount)
            else:
                monthly_revenue[month_key]["outstanding"] += float(amount)
            ...
            customer_revenue[customer_id]["total_revenue"] += float(amount)
            customer_revenue[customer_id]["invoice_count"] += 1
            
            if invoice.status == InvoiceStatus.PAID:
                customer_revenue[customer_id]["paid_amount"] += float(amount)
            else:
                customer_revenue[customer_id]["outstanding_amount"] += float(amount)
        
        # Calculate rates
        collection_rate = (paid_revenue / total_revenue * 100) if total_revenue > 0 else 0
        overdue_rate = (overdue_revenue / total_revenue * 100) if total_revenue > 0 else 0
        ...
        return {
            ...
            "monthly_breakdown": list(monthly_revenue.values()),
    ...
    async def get_expense_analytics(
        ...
        for expense in expenses:
            ...
            by_category[category]["average_amount"] = (
                by_category[category]["total_amount"] / by_category[category]["expense_count"]
            )
        ...
        reimbursement_rate = (reimbursable_amount / total_amount * 100) if total_amount > 0 else 0
    ...
    async def get_profitability_analytics(
        ...
        for month_data in monthly_profit.values():
            if month_data["revenue"] > 0:
                month_data["margin"] = (month_data["profit"] / month_data["revenue"] * 100)
        ...
        return {
            ...
            "monthly_breakdown": sorted(monthly_profit.values(), key=lambda x: x["month"])
        }
