...
# Use dateutil for accurate month math (handles month lengths, year rollover)
try:
    from dateutil.relativedelta import relativedelta
# fallback if dateutil isn't available (approx 30 days / month)
except Exception:
    relativedelta = None
...
async def get_financial_dashboard(
    ...
    current_user: CurrentUser = Depends(
        require_permission("finance", "read", "analytics")
    ),
):
    """Get financial dashboard overview."""
    return await AnalyticsService(session).get_financial_dashboard()
...
async def get_revenue_analytics(
    ...
    """Revenue analytics for the last N months (default 12)."""
    return await AnalyticsService(session).get_revenue_analytics(
...
async def get_expense_analytics(
    period_months: int = Query(12, ge=1, le=120, description="How many months back"),
    currency: str = Query("MAD"),
    session: Session = Depends(get_session),
    current_user: CurrentUser = Depends(
        require_permission("finance", "read", "analytics")
    ),
):
    """Same conversion for expenses analytics."""
    end_date = date.today()
    if relativedelta:
    ...
    return await analytics_service.get_expense_analytics(start_date, end_date, currency)
...
async def get_aging_report(
    report_type: str = Query(
        "receivables", description="Type: receivables or payables"
    ),
    session: Session = Depends(get_session),
    current_user: CurrentUser = Depends(
        require_permission("finance", "read", "analytics")
...
async def export_profit_loss_statement(
    ...
    session: Session = Depends(get_session),
    current_user: CurrentUser = Depends(
        require_permission("finance", "export", "reports")
    ),
):
    ...
    # Generate report
    report_buffer, content_type, filename = (
        await analytics_service.export_profit_loss_statement(
...
async def export_balance_sheet(
    ...
    # Generate report
    report_buffer, content_type, filename = (
        await analytics_service.export_balance_sheet(as_of_date_parsed, format)
    )
...
async def export_cash_flow_statement(
    ...
    # Generate report
    report_buffer, content_type, filename = (
        await analytics_service.export_cash_flow_statement(
            start_date_parsed, end_date_parsed, format
        )
    )
