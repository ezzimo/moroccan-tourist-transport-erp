...
async def add_itinerary_item(
    ...
    current_user: CurrentUser = Depends(require_permission("tours", "create", "itinerary"))
):
...
async def update_itinerary_item(
    ...
    session: Session = Depends(get_session),
    redis_client: redis.Redis = Depends(get_redis),
    current_user: CurrentUser = Depends(require_permission("tours", "update", "itinerary"))
):
...
async def complete_itinerary_item(
    ...
    itinerary_service = ItineraryService(session, redis_client)
    return await itinerary_service.complete_item(item_id, completion_data, current_user.user_id)
...
async def delete_itinerary_item(
    ...
    session: Session = Depends(get_session),
    redis_client: redis.Redis = Depends(get_redis),
    current_user: CurrentUser = Depends(require_permission("tours", "delete", "itinerary"))
):
...
@router.get("/tour/{tour_instance_id}", response_model=List[ItineraryItemResponse])
...
async def get_day_itinerary(
    ...
    itinerary_service = ItineraryService(session, redis_client)
    return await itinerary_service.get_day_itinerary(tour_instance_id, day_number)
...
async def reorder_day_items(
    ...
    itinerary_service = ItineraryService(session, redis_client)
    return await itinerary_service.reorder_items(tour_instance_id, day_number, item_order)