...
class BookingService:
    ...
    async def create_booking(self, booking_data: BookingCreate, current_user_id: uuid.UUID) -> BookingResponse:
        """Create a new booking with locking mechanism"""
        # Acquire lock for booking creation
        lock_key = f"booking_create:{booking_data.customer_id}:{booking_data.start_date}"
    ...
    async def get_bookings(
        ...
        if search:
            ...
            if search.lead_passenger_email:
                conditions.append(
                    Booking.lead_passenger_email.ilike(f"%{search.lead_passenger_email}%")
                )
    ...
    async def update_booking(self, booking_id: uuid.UUID, booking_data: BookingUpdate) -> BookingResponse:
    ...
    async def confirm_booking(self, booking_id: uuid.UUID, confirm_data: BookingConfirm) -> BookingResponse:
