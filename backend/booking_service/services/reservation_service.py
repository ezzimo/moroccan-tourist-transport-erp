...
class ReservationService:
    ...
    async def add_reservation_item(self, item_data: ReservationItemCreate) -> ReservationItemResponse:
    ...
    async def get_booking_items(self, booking_id: uuid.UUID) -> List[ReservationItemResponse]:
    ...
    async def update_reservation_item(
        ...
        # Update fields
        update_data = item_data.model_dump(exclude_unset=True, exclude={"specifications"})
    ...
    async def confirm_reservation_item(self, item_id: uuid.UUID) -> ReservationItemResponse:
    ...
    async def cancel_reservation_item(self, item_id: uuid.UUID) -> ReservationItemResponse:
    ...
    async def get_booking_summary(self, booking_id: uuid.UUID) -> dict:
        ...
        summary = {
            ...
            "pending_items": len([item for item in items if not item.is_confirmed and not item.is_cancelled]),
            "total_value": sum(item.total_price for item in items if not item.is_cancelled),
            "by_type": {}
        }
        ...
        for item_type in summary["by_type"]:
            summary["by_type"][item_type]["total_value"] = float(summary["by_type"][item_type]["total_value"])
