...
from schemas.availability import (
    AvailabilityRequest, AvailabilityResponse, ResourceAvailability,
...
class AvailabilityService:
    ...
    async def create_availability_slot(self, slot_data: AvailabilitySlotCreate) -> AvailabilitySlotResponse:
    ...
    async def get_availability_summary(
        ...
        summary = {
            ...
            "available_slots": len([s for s in slots if s.available_capacity > 0 and not s.is_blocked]),
            "blocked_slots": len([s for s in slots if s.is_blocked]),
            "fully_booked_slots": len([s for s in slots if s.available_capacity == 0 and not s.is_blocked]),
            "total_capacity": sum(s.total_capacity for s in slots),
            "available_capacity": sum(s.available_capacity for s in slots if not s.is_blocked),
        ...
        for slot in slots:
            ...
            summary["by_resource_type"][resource_type_key]["total_capacity"] += slot.total_capacity
            
            if slot.available_capacity > 0 and not slot.is_blocked:
                summary["by_resource_type"][resource_type_key]["available_slots"] += 1
                summary["by_resource_type"][resource_type_key]["available_capacity"] += slot.available_capacity
