...
class AvailabilitySlot(SQLModel, table=True):
    ...
    resource_type: Optional[ResourceType] = Field(index=True, default=None)
    # Reference to external service
    resource_id: Optional[uuid.UUID] = Field(index=True, default=None)
    resource_name: Optional[str] = Field(max_length=255, default=None)

    # Availability Details
    date: Optional[date] = Field(index=True, default=None)
    ...
    created_at: Optional[datetime] = Field(
        default_factory=lambda: datetime.now(timezone.utc, default=None)
    )
