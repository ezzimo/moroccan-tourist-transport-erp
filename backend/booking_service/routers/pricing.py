...
async def create_pricing_rule(
    ...
    session: Session = Depends(get_session),
    current_user: CurrentUser = Depends(require_permission("booking", "create", "pricing"))
):
...
async def get_pricing_rules(
    ...
    rule_responses = []
    for rule in rules:
        rule_responses.append(PricingRuleResponse(
            id=rule.id,
            name=rule.name,
            description=rule.description,
            code=rule.code,
...
async def update_pricing_rule(
    ...
    session: Session = Depends(get_session),
    current_user: CurrentUser = Depends(require_permission("booking", "update", "pricing"))
):
...
async def delete_pricing_rule(
    rule_id: uuid.UUID,
    session: Session = Depends(get_session),
    current_user: CurrentUser = Depends(require_permission("booking", "delete", "pricing"))
):
