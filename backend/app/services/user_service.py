...
class UserService:
    ...
    async def create_user(self, user_data: UserCreate, actor_id: Optional[uuid.UUID] = None) -> UserResponse:
        """Create a new user"""
    ...
    async def search_users(
        ...
        if filters.role_ids:
            count_query = count_query.join(UserRole).where(UserRole.role_id.in_(filters.role_ids))
    ...
    async def update_user(self, user_id: uuid.UUID, user_data: UserUpdate, actor_id: Optional[uuid.UUID] = None) -> UserResponse:
        ...
        if actor_id:
            ...
            if changes or role_ids is not None:
                await self._log_activity(
                    ...
                    metadata={"changes": changes, "role_ids": [str(rid) for rid in (role_ids or [])]}
                )
        
        return UserResponse.model_validate(user)
    
    async def delete_user(self, user_id: uuid.UUID, actor_id: Optional[uuid.UUID] = None, hard_delete: bool = False) -> dict:
    ...
    async def assign_roles(self, user_id: uuid.UUID, role_ids: List[uuid.UUID], actor_id: Optional[uuid.UUID] = None) -> dict:
    ...
    async def lock_user_account(self, user_id: uuid.UUID, actor_id: Optional[uuid.UUID] = None) -> dict:
        """Lock user account"""
        statement = select(User).where(User.id == user_id)
        user = self.session.exec(statement).first()
    ...
    async def unlock_user_account(self, user_id: uuid.UUID, actor_id: Optional[uuid.UUID] = None) -> dict:
        """Unlock user account"""
        statement = select(User).where(User.id == user_id)
        user = self.session.exec(statement).first()
    ...
    async def reset_password(self, user_id: uuid.UUID, actor_id: Optional[uuid.UUID] = None, new_password: Optional[str] = None) -> dict:
        """Reset user password"""
        statement = select(User).where(User.id == user_id)
        user = self.session.exec(statement).first()
    ...
    async def get_user_activity(self, user_id: uuid.UUID, limit: int = 50) -> List[Dict[str, Any]]:
